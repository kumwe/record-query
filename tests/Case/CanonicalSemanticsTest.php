...
final class CanonicalSemanticsTest extends TestCase
{
    public function testQueryDigestMatchesFrozenSdkBytes(): void
    {
        ...
        $this->assertSame(
            $proof['default_query']['document'],
            $query->toArray(false),
            'Canonical query plan retains every field.'
        );
        $this->assertSame(
            $proof['default_query']['digest'],
            $query->digest(),
            'Extracted query digest retains exact SDK bytes.'
        );
    }
    public function testNarrowLiteralAdmissionPrecedesCanonicalRecordReduction(): void
    {
        $this->assertSame(
            '2024-02-29T10:00:00.123456+02:00',
